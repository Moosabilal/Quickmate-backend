...
export class AuthService implements IAuthService {
  ...
  public async resendOtp(data: ResendOtpRequestBody): Promise<{ message: string }> {
    ...
    if (user.registrationOtpExpires && user.registrationOtpExpires instanceof Date) {
      const timeSinceLastOtpSent =
        Date.now() - (user.registrationOtpExpires.getTime() - OTP_EXPIRY_MINUTES * 60 * 1000);
  ...
  public async login(
    ...
    if (!user.isVerified) {
      throw new CustomError(
        `Account not verified. Please verify your email or contact admin.`,
  ...
  public async googleAuthLogin(token: string): Promise<{
  ...
  public async createRefreshToken(refresh_token: string): Promise<{ newToken: string }> {
    try {
      ...
      const newToken = jwt.sign({ id: user._id, role: user.role }, process.env.JWT_SECRET as string, {
        expiresIn: "1h",
      });
      return { newToken };
    } catch {
  ...
  public async getUserWithAllDetails(
    page: number,
    limit: number,
    search: string,
    status: string,
