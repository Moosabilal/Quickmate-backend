...
export const startBookingExpiryJob = () => {
    cron.schedule("1 0 * * *", async () => {
        ...
        const bookingRepo = container.get(TYPES.BookingRepository);
        const walletRepo = container.get(TYPES.WalletRepository);
        const providerRepo = container.get(TYPES.ProviderRepository);
        try {
            ...
            for (const booking of overdueBookings) {
                ...
                const updateData = { status: BookingStatus.EXPIRED };
                ...
                if (booking.providerId) {
                    ...
                    if (provider) {
                        ...
                        logger.info(`📉 [Cron] Penalized provider ${providerId}. Rating dropped from ${currentRating} to ${newRating}.`);
                        if (provider.email) {
                            await sendPenaltyEmail(provider.email, booking.scheduledDate, newRating);
                        }
                    }
                }
